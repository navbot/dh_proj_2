...
sentiment_Amory <- entities_matches_sentiment[str_detect(entities_matches_sentiment$words, "Amor"), ] %>% filter(title == "This Side of Paradise") %>%
  group_by(title, words, kind) %>%
  summarise(across(anger:sentiment, sum)) %>% select(-positive, -negative, -sentiment, -kind)
...
sentiment_Henry <- entities_matches_sentiment[str_detect(entities_matches_sentiment$words, "Henry"), ] %>% filter(title == "A Farewell to Arms") %>%
  group_by(title, words, kind) %>%
  summarise(across(anger:sentiment, sum)) %>% select(-positive, -negative, -sentiment, -kind)

all_protag <- bind_rows(sentiment_Gatsby, sentiment_Anthony, sentiment_Amory, sentiment_Dick_Diver) %>% mutate(total = rowSums(across(where(is.numeric)))) %>% mutate(across(anger:trust, .fns = ~ round((. / total) * 100)))
...
all_protag_h <- bind_rows(sentiment_Henry, sentiment_Robert, sentiment_Jake, sentiment_David) %>% mutate(total = rowSums(across(where(is.numeric)))) %>% mutate(across(anger:trust, .fns = ~ round((. / total) * 100)))

ggRadar(
  # Add select(-total)
  data = all_protag_h %>% select(-total),
  mapping = aes(color = title, facet = words),
  rescale = FALSE,
  interactive = TRUE,
  use.label = TRUE,
