...
my_text <- read_file("The Great Gatsby_Fitzgerald, F. Scott (Francis Scott)_clean.txt")
...
#counting how often female words appeared
#table shows each text and the respective percentage
fitzgerald_emotion_calc <- fitzgerald_emotions %>% group_by(title) %>% count(emotion) %>% mutate(percent = n/sum(n)*100)
...
#counting how often female words appeared
#table shows each text and the respective percentage
hemingway_emotion_calc <- hemingway_emotions %>% group_by(title) %>% count(emotion) %>% mutate(percent = n/sum(n)*100)
